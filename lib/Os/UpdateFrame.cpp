...
static void DrawPheromon(int k, int j)
{
	...
	for (int cl = 0; cl < MData.nb_clan; ++cl)
		for (int hk = 0; hk < 1; hk++)
		{
			...
			{
				if ((k + ZSF.column == ZSF.This_room()->Gate[0][hk]) && (j + ZSF.line == ZSF.This_room()->Gate[1][hk]))
				{
					rc.setSFRect(7 * 32, 600, 7 * 32 + 32, 632);
					SFDrawSurface(160 + k * 32, j * 32, rc);
				}
			}
			else if ((k + ZSF.column == les_clans[cl]->Gate[0][hk]) && (j + ZSF.line == les_clans[cl]->Gate[1][hk]))
			{
...
static void DrawAnimal(int k, int j, int F)
{
	...
	if ((ani = TEST_ANIMAL(k + ZSF.column, j + ZSF.line, ZSF.This_room())) != NULL)
	{
		...
		case FOURMI:
			...
			GraphPos[F][0] = -(ani->Xc - ani->Oldpos[0][0] * 32) * (MData.fps - F - 1) / MData.fps;
			GraphPos[F][1] = -(ani->Yc - ani->Oldpos[0][1] * 32) * (MData.fps - F - 1) / MData.fps;
			SFDrawSurface(160 + 32 * k + GraphPos[F][0], 32 * j + GraphPos[F][1], rc);
...
static void DrawOutset(int k, int j)
{
	...
	{
		if (ZSF.This_room()->map[k + ZSF.column][j + ZSF.line].decor->etat < MData.max_meat / 3)
			rc.setSFRect(0, 5 * 32 + 600, 32, 5 * 32 + 632);
		else if (ZSF.This_room()->map[k + ZSF.column][j + ZSF.line].decor->etat > 2 * MData.max_meat / 3)
