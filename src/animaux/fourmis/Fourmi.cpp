...
Fourmi::Fourmi(int x, int y, type social, int num, Fourmi *reine, CRoom *room) : CAnimal(x, y, FOURMI, num, room)
{
...
int Fourmi::RegardeAnimalAutour(int Peri, bool cl, enum espece esp)
// Paramètres:
...
int Fourmi::RegardeNourritureAutour(int Peri) const
// Paramètres:
// Peri   : périmètres de vision
...
int Fourmi::ChangeRoom(class CRoom *rm)
// Paramètres
// rm	: room où l'on va
...
// 1	: sortie sans problème
{
  ...
  {
    dir = 2 * (rand() % 8);
    x = Xc + CAnimal::Deplacements[dir % 16];
    y = Yc + CAnimal::Deplacements[(dir % 16) + 1];
    if (CASE_VIDE(x, y, rm))
      break;
  }
  if (i == 20)
...
// ou NULL si aucune trouvée.
{
  ...
  for (int i = 0; i < MData.max_ants; i++)
  {
    ...
    if (fourmi_tmp == this || fourmi_tmp->Statut() != defourmi || fourmi_tmp->Room != this->Room)
      continue;
...
// NUL	:sinon
{
  ...
  if (!CASE_VALIDE(x, y, this->Room))
    return NUL;

  if (Room->map[x][y].phero[categ]->Genre == NUL)
    return NUL;
  if (Room->map[Xc][Yc].phero[categ]->Vex == -Room->map[x][y].phero[categ]->Vex && Room->map[Xc][Yc].phero[categ]->Vey == -Room->map[x][y].phero[categ]->Vey)
    return NUL;
...
// dans Dest	:la position de ces phéromones
{
  ...
  for (int x = Xc - 1; x <= Xc + 1; x++)
    ...
    {
      if (!CASE_VALIDE(x, y, Room))
        continue;

      if (Room->map[x][y].phero[categ]->Action() != OK)
        continue;

      if (x == Xc && y == Yc)
        return ON;

      if (Room->map[x][y].phero[categ]->Intensite > Room->map[xn][yn].phero[categ]->Intensite)
      {
        xn = x;
        yn = y;
      }
...
int Fourmi::Lacher_Pheromone(categorie cat)
// Paramètres
...
// 1		:OK
{
  ...
  for (x = Xc - 1; x <= Xc + 1; x++)
    for (y = Yc - 1; y <= Yc + 1; y++)
    {
      if (!CASE_VALIDE(x, y, Room))
        continue;
      if (Room->map[x][y].decor->genre == MUR)
        continue;
      ...
      {
      case NOURRITURE:
        if (Oldpos[0][0] == Xc && Oldpos[0][0] == Yc)
          break;
...
void Fourmi::Attaque(CAnimal *ani, int force) const
// Paramètres
...
int Fourmi::EnterTunnel(const class CTunnel *Tunnel)
...
bool Fourmi::Serialize(FILE *fichier, bool sauv, CClan *clan, IOTablePointeur &table)
// Paramètres
...
// sauv		: si true sauve, si false charge
{
  ...
  if (sauv && fichier != NULL) // En cas de sauvegarde
  {
    ...
    if (!fwrite(&i, sizeof(int), 1, fichier))
    { // Sauve le numero de clan dans la table
      SDEBUG(W4, "Erreur d'écriture");
      return false;
    }
    i = table.FindOrAdd((void *)this->PCiterne);
    if (!fwrite(&i, sizeof(int), 1, fichier))
    { // Sauve le numero de pCiterne dans la table
      SDEBUG(W4, "Erreur d'écriture");
      return false;
    }
    i = table.FindOrAdd((void *)this->prev);
    if (!fwrite(&i, sizeof(int), 1, fichier))
    { // Sauve le numero de prev dans la table
      SDEBUG(W4, "Erreur d'écriture");
      return false;
    }
    i = table.FindOrAdd((void *)this->next);
    if (!fwrite(&i, sizeof(int), 1, fichier))
    { // Sauve le numero de next dans la table
      SDEBUG(W4, "Erreur d'écriture");
      return false;
    }
  ...
  {
    if (fichier != NULL)
    { // Lecture du fichier, pas de mise à jour des pointeurs
      if (!fread(&i, sizeof(int), 1, fichier))
      ...
      if (!fread(&this->Room, sizeof(int), 1, fichier))
      { // room, clan, pCiterne, prev, next ->i,j,k,l,m
        SDEBUG(W4, "Erreur de lecture");
        return false;
      }
      if (!fread(&this->Clan, sizeof(int), 1, fichier))
      { // room, clan, pCiterne, prev, next ->i,j,k,l,m
        SDEBUG(W4, "Erreur de lecture");
        return false;
      }
      if (!fread(&this->PCiterne, sizeof(int), 1, fichier))
      { // room, clan, pCiterne, prev, next ->i,j,k,l,m
        SDEBUG(W4, "Erreur de lecture");
        return false;
      }
      if (!fread(&this->prev, sizeof(int), 1, fichier))
      { // room, clan, pCiterne, prev, next ->i,j,k,l,m
        SDEBUG(W4, "Erreur de lecture");
        return false;
      }
      if (!fread(&this->next, sizeof(int), 1, fichier))
      { // room, clan, pCiterne, prev, next ->i,j,k,l,m
        SDEBUG(W4, "Erreur de lecture");
        return false;
      }
