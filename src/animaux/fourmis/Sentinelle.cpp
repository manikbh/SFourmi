...
Sentinelle::Sentinelle(int x, int y, int num, Fourmi *reine, CRoom *room) : Fourmi(x, y, SENTINELLE, num, reine, room)
{
  ...
  for (x = Xc - 1; x <= Xc + 1; x++)
    for (y = Yc - 1; y <= Yc + 1; y++)
    {
      ...
      if (TEST_ANIMAL(x, y, Room) != NULL)
        ...
        if (Room->map[x][y].firstAnimal->Esp == CAFARD)
          c++;
    }
  return c;
}
