...
void User::Initialize(class DataMap &MData)
{
  ConstructEnvironment(MData);
}

void User::ConstructEnvironment(class DataMap &MData)
{
  srand(time(NULL));
...
void User::CheckWall(class DataMap &MData)
{
  int ci = 1;
  int sum = 0;
  ...
  for (int i = 0; i < MData.length; i++)
    for (int j = 0; j < MData.height; j++)
      if (terre->map[i][j].decor->genre == MUR)
      {
        ...
        {
        case (1 + 2):
          terre->map[i][j].terrain = 18;
          break;
        case (1 + 4):
          terre->map[i][j].terrain = 12;
          break;
        case (2 + 8):
          terre->map[i][j].terrain = 16;
          break;
        case (4 + 8):
          terre->map[i][j].terrain = 10;
          break;
        case (1 + 2 + 4):
          terre->map[i][j].terrain = 15;
          break;
        case (2 + 4 + 8):
          terre->map[i][j].terrain = 13;
          break;
        case (1 + 2 + 8):
          terre->map[i][j].terrain = 17;
          break;
        case (1 + 4 + 8):
          terre->map[i][j].terrain = 11;
          break;
        case (1 + 2 + 4 + 8):
          terre->map[i][j].terrain = 14;
          break;
        }
      }
}

void User::Init_Clan(class DataMap &MData)
{
  CRoom *tmpRoom;
  CTunnel *firsttunnel;
  int i, j;
...
void User::Init_Queens(class DataMap &MData)
{
  int i, j;
...
void User::Init_Workers(class DataMap &MData)
{
  int i, j;
...
void User::Init_Guards(class DataMap &MData)
{
  int i, j;
...
void User::Init_Tanks(class DataMap &MData)
{
  CClan *tmpcl;
  int i, j;
...
void User::Serialize(FILE *fichier, bool sauv, class IOTablePointeur &table)
{
  ...
  { // Chargement
    if (fichier == NULL)
    { // Dans ce cas, lire la table de pointeurs
    ...
    { // Chargement première étape : les pointeurs restent vides.
...
void User::Init_Ants(class DataMap &MData)
{
  /// Reine ///
  SDEBUG(W0, "->Reines<-");
  Init_Queens(MData);
...
void User::Init_Bugs(class DataMap &MData)
{
  int i, j;
...
void User::Init_Ennemies(class DataMap &MData)
{
  /// Ennemis ///
  SDEBUG(W0, "->Ennemis<-");
