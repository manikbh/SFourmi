...
void CRoom::Init(int largeur, int hauteur, typeRoom tR)
// Paramètres
...
void CRoom::SetEntrees(int x, int y, int num)
// Paramètres
// x		:Entrée x
// y		:Entrée y
...
void CRoom::AjouteEntree(int x, int y, CClan *clan)
{
...
bool CRoom::Serialize(FILE *sauveg, bool sauv, IOTablePointeur &table)
{
	int i, j, k2;
	unsigned k;
	if (sauv && sauveg != NULL) // Sauvegarde
	{
		...
		for (int k = 0; k < size.largeur; k++) // On parcourt le terrain
			for (int k2 = 0; k2 < size.hauteur; k2++)
			{
				...
				fflush(sauveg);
				if ((i = table.Find((void *)(cd = ca.decor))) == -1)
				{ // Si le décor n'est pas encore sauvé
